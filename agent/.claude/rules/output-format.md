...
## recommendation  
{status, message (plain text), data: {components: [{ref, mpn, manufacturer, description, package, qty_per_unit, qty_total, justification, unit_price, price_break, stock, lifecycle, distributor, distributor_url, datasheet_url, mpn_confidence, verified, warnings, alternatives}], not_sourced, bom_summary: {unique_parts, total_components_per_unit, cost_per_unit, cost_total, volume, currency}, export_files: {csv, kicad_library, altium_library}, sources_queried}}
...
- Prices in PLN (TME default) or as returned by the API
